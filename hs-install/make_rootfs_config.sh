function make_rootfs_config {
    ...
    if $_ubuntu
    then
       echo "nameserver 10.$HOSTNET.0.1" > $rootfs/etc/resolvconf/resolv.conf.d/tail
    fi
...
search local
nameserver 10.$HOSTNET.0.1
"
    fi
    
    if $_ubuntu
    then
        echo "nameserver 10.$HOSTNET.0.1" >> $rootfs/etc/resolvconf/resolv.conf.d/base
    fi
